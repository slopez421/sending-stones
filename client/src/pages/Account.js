...
function Account({user, setUser, accountUpdated, setAccountUpdated}) {
...
const [isUpdating, setIsUpdating] = useState(false)
    ...
    return <div className="grid place-content-center mt-20">
        <div className="card-normal max-w-sm shrink-0 shadow-2xl rounded-box bg-primary text-primary-content w-96 ">
            {isUpdating ? (
            <UpdateAccountForm accountUpdated={accountUpdated} setAccountUpdated={setAccountUpdated} user={user} setUser={setUser} setIsUpdating={setIsUpdating}/>
             )
            : 
